...
function ____exports.reduceGraph(self, graph)
    ...
    while nonReducedCount < reduced.size do
        do
            local i = 0
            while i < reduced.size do
                local n = reduced:get(i)
                if n == nil then
                    error(
                        tostring(i) .. " was null"
                    )
                end
                if #n.neighbours == 2 then
                    local nAId = n.neighbours[1]
                    local nA = reduced:get(nAId)
                    local nBId = n.neighbours[2]
                    local nB = reduced:get(nBId)
                    if nA == nil then
                        error(
                            tostring(nAId) .. " is backed by null"
                        )
                    end
                    if nB == nil then
                        error(
                            tostring(nBId) .. " is backed by null"
                        )
                    end
                    if ((nA.pos.x == n.pos.x) and (nB.pos.x == n.pos.x)) or ((nA.pos.z == n.pos.z) and (nB.pos.z == n.pos.z)) then
                        nA.neighbours = __TS__ArrayFilter(
                            nA.neighbours,
                            function(____, it) return it ~= i end
                        )
                        nB.neighbours = __TS__ArrayFilter(
                            nB.neighbours,
                            function(____, it) return it ~= i end
                        )
                        __TS__ArrayPush(nA.neighbours, nBId)
                        __TS__ArrayPush(nB.neighbours, nAId)
                        reduced:set(nAId, nA)
                        reduced:set(nBId, nB)
                        reduced:delete(i)
                    else
