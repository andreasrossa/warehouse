...
export type NodeGraph = Map<Pos2D, GraphNode>;
...
export function toGlobalPos(offset: number[], globalPos: Position3D): Position3D {
    let newPos = [];
    for (let i = 0; i < 3; i++) {
...
export const width = (r: Rectangle): number => Math.abs(r.bottomRight[0] - r.bottomLeft[0]) + 1;
export const length = (r: Rectangle): number => Math.abs(r.topRight[2] - r.bottomRight[2]) + 1;
export const getPos = (nav: OpenOS.Navigation): Position3D => [...nav.getPosition()];
...
export function print3DMatrix(matrix: Matrix3D) {
    term.clear();
...
export function printNodeGraph(graph: NodeGraph) {}
...
export function nodeGraphFromPositions(nodePositions: Pos2D[]): NodeGraph {
