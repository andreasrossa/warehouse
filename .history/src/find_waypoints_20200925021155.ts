...
type NodeGraph = Map<Position2D, Node>;

function exploreSides(currentPos: Position2D, graph: NodeGraph, queue: number[], matrix: Matrix2D) {
	const exists = graph.has(currentPos)

	if(exists) {
		if(queue.length > 0) {
			const g = [...graph].find(([pos, node]) => node.index == queue[queue.length - 1])!!;
			if(g == null) return
			const [pos, n] = g
			exploreSides
		} else {
	...
	if(top == 0 && graph) sides.push({pos: [pos[0], pos[1]+1], neighbours: [currentNode]})
