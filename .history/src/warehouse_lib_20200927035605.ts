...
export function getNeighbours(currentPos: Pos2D, graph: NodeGraph): number[] {
    return [...graph].filter(
        ([id, n]) =>
...
export function nodeGraphFromPositions(nodePositions: NodePosMap): PosNodeGraph {
...
export function reduceGraph(nodePositions: NodePosMap): PosNodeGraph {
