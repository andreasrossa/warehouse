...
export function nodeGraphFromPositions(nodePositions: NodePosMap): NodeGraph {
    ...
    for (let i = 0; i < nodePositions.size; i++) {
        ...
        graph.set(graph.size, { neighbours: neighbours, pos: pos });
    }
    ...
    while (nonReducedCount < reduced.size) {
        reduced.forEach((n, id) => {
            if (
                ...
                ((graph.get(n.neighbours[0])?.pos.x == n.pos.x && graph.get(n.neighbours[1])?.pos.x == n.pos.x) ||
                    (graph.get(n.neighbours[0])?.pos.z == n.pos.z && graph.get(n.neighbours[1])?.pos.z == n.pos.z))
            ) {
                const nAPos = id.neighbours[0];
                const nA = reduced.get(nAPos)!!;
                const nBPos = id.neighbours[1];
                const nB = reduced.get(nBPos)!!;

                nA.neighbours = nA.neighbours.filter((it) => it != n);
                nB.neighbours = nB.neighbours.filter((it) => it != n);
