 ...
 */
function scanRectangle(r: Rectangle, nav: OpenOS.Navigation) {
	const pos = getPos(nav)
	//TODO: Scan area in 64 block chunks to cover bigger areas
	if(width(r) * length(r) > 64) error("Maximum volume is 64 blocks (for now)") 

	const a = r.bottomLeft
	const relativeOrigin = [a[0] + pos[0], pos[1], a[2] + pos[2]]
}