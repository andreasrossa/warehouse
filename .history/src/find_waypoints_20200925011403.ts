    ...
    for (let y = 0; y < matrix.length; y++) {
        for (let z = 0; z < matrix[y].length; z++) {
            ...
            for(let x = matrix[y][z].length - 1; x >= 0; x--) {
				gpu.set(x+cursorOffset[0],z+cursorOffset[1],matrix[y][z][x] > 0 ? "O" : " ")
			}
        }
...
type NodeGraph = { [index: number]: Node }
...
function nodeGraphFromMatrix(matrix: Matrix2D, startPoint: Position2D) {
	...
	if(matrix[startPoint[1]][startPoint[0]] != 0) error("Start-Block is not air")
