 ...
 */
export function getWaypoints(waypointLabels: string[], nav: OpenOS.Navigation): { [name: string]: OpenOS.Waypoint } {
 ...
 */
function scanRectangle(r: Rectangle, nav: OpenOS.Navigation, geo: OpenOS.Geolyzer): Matrix3D {
	const pos = getPos(nav)
	const rW = width(r);
	const rL = length(r);
...
function scanWaypointArea(waypointA: string, waypointB: string, nav: OpenOS.Navigation, geo: OpenOS.Geolyzer) {
	const waypoints = getWaypoints([waypointA, waypointB], nav)
	const r = getRectangle(waypoints[waypointA], waypoints[waypointB], nav)
